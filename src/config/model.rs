...
use super::{
    ...
    DEFAULT_MOUSE_SCROLL_LINES, DEFAULT_SCROLLBACK_LIMIT_BYTES, DEFAULT_STATUS_INTERVAL_SECONDS,
    DEFAULT_STATUS_RIGHT_LENGTH, MIN_STATUS_INTERVAL_SECONDS,
};
...
"#;
    ...
    fn status_config_defaults_disable_strip() {
        ...
        assert!(!crate::app::status_strip::StatusStripState::from_config(
...
"##;
        ...
        assert!(
            crate::app::status_strip::StatusStripState::from_config(&config.ui.status).is_enabled()
        );
...
"#;
        let config: Config = toml::from_str(toml).expect("valid config");
        assert!(
            !crate::app::status_strip::StatusStripState::from_config(&config.ui.status)
                .is_enabled()
        );
    }
