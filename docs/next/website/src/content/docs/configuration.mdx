...
`command_palette` (default `prefix+:`) opens a floating command palette that fuzzy-filters a live catalog of commands and runs the one you pick through Herdr's normal execution path. Type to narrow the list, `↑`/`↓` (or `ctrl+p`/`ctrl+n`) to move, `↵` to run, and `esc` to cancel. Unbind it (`command_palette = ""`) to disable the feature entirely.

The catalog merges three sources, each shown with a dim right-aligned tag: built-in navigation and pane commands (`built-in`), actions contributed by installed plugins (`plugin`), and your `[[keys.command]]` custom commands (`custom`). An arg-taking built-in such as `rename-workspace` or `workspace-picker` opens its existing overlay; a no-arg command such as `zoom` runs and closes the palette.

Choose which sources feed the palette with `[command_palette].sources` (all enabled by default):

```toml
[command_palette]
sources = { built_in = true, plugin = true, custom = true }
```

The native palette **supersedes the `jt.command-palette` plugin** — unlink it with `herdr plugin unlink jt.command-palette` to avoid a duplicate. The separate `local.smart-send` plugin is unrelated and continues to work.
...
An optional `label` can be provided. When set, the command palette shows the label as the entry name and moves the chord into the right-aligned keybind column. When omitted, the chord itself is the entry name and that column stays blank.
...
`[ui.status]` adds a tmux-style right-aligned status strip to the tab bar. `status_right` is the format string; it is empty by default, which hides the strip. It supports literal text, a `%`-strftime subset (`%H`, `%M`, `%S`, `%d`, `%b`, `%Y`, and `%%`), and `#(command)` segments whose last line of standard output becomes the segment text, matching how `tab_bar_right` command entries read their output. A segment's text is capped at 80 characters on ingest, before `status_right_length` budgeting, so a longer command output or pushed value is shortened even when the strip has room for it. Simple single-line tmux status scripts such as `#(cpu.sh)` or `#(gitmux)` work nearly verbatim. `status_right_length` is the column budget reserved on the right of the tab bar (default 28); the tabs zone flexes into the remainder and scrolls first, and when the strip still overflows it drops whole segments from the left. The strip and the `tab_bar_right` entries decorate the same right edge, so a configured strip takes it and the `tab_bar_right` entries are not drawn. `status_interval` is the refresh cadence in seconds for `#(command)` segments (default 5, floored at 1). The clock repaints on its own finer cadence, so a `%S` clock stays current regardless of `status_interval`. A `#(command)` or clock segment that resolves to an empty string also drops its adjacent separator, so a temporarily blank segment leaves no dangling ` │ `. The strip is hidden in the mobile single-column layout.

`status_right` also accepts tmux-style `#[…]` style directives that color the segments that follow them. A directive sets the current style and stays in effect until the next directive or the end of the string. Supported attributes, comma-combinable inside one `#[…]`, are `fg=<color>`, `bg=<color>`, the modifiers `bold`, `dim`, `italic`, `underline`, and `reverse`, and `default` (or `none`) which resets to the strip's base style. `<color>` uses the same parser as `accent`: a name (`green`), hex (`#1e1e2e`), or `rgb(r,g,b)`. Unknown attributes and unparseable colors are ignored. Directives have zero display width, so they never count against `status_right_length` or truncation. Style comes only from the format string you author; `#(command)` output stays sanitized plain text and cannot inject its own colors. A powerline-style example:

```toml
[ui.status]
status_right = "#[fg=black,bg=green] #(gitmux) #[fg=green,bg=blue] %H:%M "
```

Besides names, hex, and `rgb(r,g,b)`, `fg` and `bg` also accept Herdr's semantic theme tokens by name: `accent`, `panel_bg`, `surface0`, `surface1`, `surface_dim`, `overlay0`, `overlay1`, `text`, `subtext0`, `mauve`, `teal`, and `peach`. These resolve from the active theme, so the strip follows your `[theme]` choice, any `[theme.custom]` overrides, and `auto_switch` light/dark switching without editing `status_right`. The ANSI names (`green`, `yellow`, `red`, `blue`, `cyan`, …) are unchanged — they still resolve to the terminal's ANSI palette rather than a theme token, so existing strips keep their appearance. A theme-token example:

```toml
[ui.status]
status_right = "#[fg=text,bg=surface0] #(gitmux) #[bg=mauve] %H:%M #[fg=accent] ●"
```

Alongside the pull lane (`#(command)`, run on `status_interval`), `status_right` supports a **push lane**: a `#{slot:NAME}` token renders the latest value a client pushed for `NAME` over the API socket, and repaints on change without any polling. Where `#(git branch)` runs wherever the server runs, a pushed slot lets an external watcher resolve a per-context value — the focused pane's directory, application, git branch, or session — and push it in. Pushed values are host-scoped (keyed by `source` only), last-writer-wins by an optional `seq`, and expire after an optional `ttl_ms`. Their text is sanitized to plain text just like `#(command)` output; styling still comes only from the `#[…]` directives you author, so a `#{slot:NAME}` can sit inside a styled pill exactly like a command segment. An unset or expired slot renders empty and drops its adjacent separator, just like an empty command. Push and clear a slot with the CLI:

```bash
herdr status set --source git --text main          # #{slot:git} now renders "main"
herdr status set --source git --text main --ttl-ms 5000   # expires after 5s
herdr status set --source git --text main --seq 7  # last-writer-wins by seq
herdr status clear --source git                    # empties #{slot:git}
```

A `status_right` mixing pull, push, and clock segments:

```toml
[ui.status]
status_right = "#[fg=mauve]#{slot:app}#[default] │ #{slot:git} │ #(cpu.sh)% │ %H:%M"
```

A shipped watcher plugin is a separate effort; the mechanism here is the building block. A minimal focused-pane git-branch watcher is just a loop that reads the focused pane's directory and pushes the branch:

```bash
# on pane focus change:
branch=$(git -C "$focused_pane_cwd" rev-parse --abbrev-ref HEAD 2>/dev/null)
herdr status set --source git --text "${branch:-}"
```
