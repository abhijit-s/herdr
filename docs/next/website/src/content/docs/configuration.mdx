...
`[ui.status]` adds a tmux-style right-aligned status strip to the tab bar. `status_right` is the format string; it is empty by default, which hides the strip. It supports literal text, a `%`-strftime subset (`%H`, `%M`, `%S`, `%d`, `%b`, `%Y`, and `%%`), and `#(command)` segments whose first line of standard output becomes the segment text. Simple single-line tmux status scripts such as `#(cpu.sh)` or `#(gitmux)` work nearly verbatim. `status_right_length` is the column budget reserved on the right of the tab bar (default 28); the tabs zone flexes into the remainder and scrolls first, and when the strip still overflows it drops whole segments from the left. `status_interval` is the refresh cadence in seconds for `#(command)` segments (default 5, floored at 1). The clock repaints on its own finer cadence, so a `%S` clock stays current regardless of `status_interval`. A `#(command)` or clock segment that resolves to an empty string also drops its adjacent separator, so a temporarily blank segment leaves no dangling ` │ `. The strip is hidden in the mobile single-column layout.
